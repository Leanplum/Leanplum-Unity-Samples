...
namespace LeanplumSDK
{
	public class LeanplumAndroid : LeanplumSDKObject
	{
		...
		{
			string jsonString = NativeSDK.CallStatic<string>("objectForKeyPath", Json.Serialize(components));
		...
		{
			string jsonString = NativeSDK.CallStatic<string>("objectForKeyPathComponents", Json.Serialize(pathComponents));
			return Json.Deserialize(jsonString);
		}
		...
		public override void Start(string userId, IDictionary<string, object> attributes, Leanplum.StartHandler startResponseAction) 
		{
			...
			NativeSDK.CallStatic("initialize", "LeanplumUnityHelper", SharedConstants.SDK_VERSION, deviceId);
