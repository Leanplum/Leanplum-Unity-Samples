...
namespace LeanplumSDK
{
	...
	public class LeanplumNative : LeanplumSDKObject
	{
		...
		/// <summary>
		///     Gets a value indicating whether Leanplum has started and the device is registered as a developer.
		/// </summary>
		/// <value><c>true</c> if Leanplum has started and the device registered as developer; otherwise, <c>false</c>.</value>
		...
		///     If you subscribe to this and variables have already been received, it will be invoked immediately.
		/// </summary>
		...
		public override void Start(string userId, IDictionary<string, object> attributes, Leanplum.StartHandler startResponseAction)
		{
			...
			parameters[Constants.Params.DEVICE_SYSTEM_VERSION] = CompatibilityLayer.GetSystemVersion();
			...
			req.Response += delegate(object responsesObject)
			{
				...
				IDictionary<string, object> values =
					Util.GetValueOrDefault(response, Constants.Keys.VARS) as IDictionary<string, object> ??
						new Dictionary<string, object>();
				IDictionary<string, object> fileAttributes =
					Util.GetValueOrDefault(response, Constants.Keys.FILE_ATTRIBUTES) as IDictionary<string, object> ??
						new Dictionary<string, object>();
				bool isRegistered = (bool) Util.GetValueOrDefault(response, Constants.Keys.IS_REGISTERED, false);
				
				LeanplumRequest.Token = Util.GetValueOrDefault(response, Constants.Keys.TOKEN) as string ?? "";
				
				// Allow bidirectional realtime variable updates.
				if (Constants.isDevelopmentModeEnabled)
				{
					VarCache.SetDevModeValuesFromServer(
						Util.GetValueOrDefault(response, Constants.Keys.VARS_FROM_CODE) as Dictionary<string, object>);
					...
					if (Constants.EnableRealtimeUpdatesInDevelopmentMode && SocketUtilsFactory.Utils.AreSocketsAvailable) {
						leanplumSocket = new LeanplumSocket(delegate() {
							// Callback when we receive an updateVars command through the development socket.
							// Set a flag so that the next time VarCache.CheckVarsUpdate() is called the variables
							// are updated.
					...
					{
						// Check for updates.
						string latestVersion = Util.GetValueOrDefault(response, Constants.Keys.LATEST_VERSION) as string;
						if (latestVersion != null)
						{
							CompatibilityLayer.Log("Leanplum Unity SDK " + latestVersion +
							                                " available. Go to https://www.leanplum.com/dashboard to download it.");
						}
		...
		public override void TrackGooglePlayPurchase(string item, long priceMicros, string currencyCode, string purchaseData,
		                                    string dataSignature, IDictionary<string, object> parameters)
		{
		...
		public override void TrackIOSInAppPurchase(string item, double unitPrice, int quantity, string currencyCode,
		                                  string transactionIdentifier, string receiptData,
		...
		///     To track purchases, use Leanplum.PURCHASE_EVENT_NAME as the event name.
		/// </summary>
		public void Track(string eventName, double value, string info, IDictionary<string, object> parameters,
		...
		public override void SetUserAttributes(string newUserId, IDictionary<string, object> value)
		{
			if (!calledStart)
			{
				CompatibilityLayer.LogWarning("Start was not called. Set user ID and attributes as the arguments " +
				                              "when calling Start.");
				return;
			}
			if (Constants.isNoop)
		...
		public override void PauseState()
		{
			...
			LeanplumRequest.Post(Constants.Methods.PAUSE_STATE, new Dictionary<string, string>()).Send();
		}
		...
		public override void ResumeState()
		{
			...
			LeanplumRequest.Post(Constants.Methods.RESUME_STATE, new Dictionary<string, string>()).Send();
		}
		...
		internal static void Pause()
		{
			...
			{
				isPaused = true;
				LeanplumRequest request = LeanplumRequest.Post(Constants.Methods.PAUSE_SESSION, null);
		...
		{
			string kind = null;
			if (defaultValue is int || defaultValue is long || defaultValue is short || defaultValue is char ||
			    defaultValue is sbyte || defaultValue is byte)
			{
		...
		private static NativeVar<U> DefineHelper<U>(string name, string kind, U defaultValue)
		{
			...
			// GetVariable(name) above will return null if the variable exists but of the wrong type.
			// Need to check if the name is not taken.
			if (VarCache.HasVariable(name))
			{
				LeanplumNative.CompatibilityLayer.LogWarning("Failed to define variable: \"" + name + "\" refers to an " +
				                                             "existing Leanplum variable of a different type.");
				return null;
			}
		...
		private static object DeepCopyContainer(object container)
		{
			...
			if (container is IDictionary)
			{
				...
				{
					((IDictionary) copied).Add(key, DeepCopyContainer(((IDictionary) container)[key]));
				}
			}
		                                                   ...
		                                                   string standaloneBundleName = "")
		{
			...
			string resourceName = Constants.Values.RESOURCES_VARIABLE + '.' + platform + " Assets." + name;
